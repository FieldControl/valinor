...
import { SwimlanesService } from '../../../../shared/services/swimlane.service';
...
export class EditSwimlaneComponent {
  ...
  swimlaneForm = this.fb.group({
    id: this.fb.control(this.data.swimlane.idSwimlane),
    name: this.fb.control(this.data.swimlane.nameSwimlane, [Validators.required]),
  });
  ...
  deleteSwimlane() {
    this.matDialog
      ...
      .pipe(
        ...
        mergeMap(() =>
          this.swimlaneService.deleteSwimlane(this.data.swimlane.idSwimlane)
        )
      )
      .subscribe(() => {

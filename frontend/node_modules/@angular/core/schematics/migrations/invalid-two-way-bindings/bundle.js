...
function migrateTwoWayEvent(value, binding, printer) {
  ...
  let migrated = null;
  if (import_typescript4.default.isBinaryExpression(expression) && isReadExpression3(expression.right)) {
  ...
  } else if (import_typescript4.default.isConditionalExpression(expression) && isReadExpression3(expression.whenFalse)) {
    ...
    while (true) {
      if (isPrefixNot(innerExpression)) {
        ...
        if (isReadExpression3(innerExpression)) {
...
function isReadExpression3(node) {
...
var InvalidTwoWayBindingCollector = class extends RecursiveVisitor {
  ...
  visitNodeWithBindings(node) {
    ...
    for (const input of node.inputs) {
      if (input.type !== 5 || seenOneWayBindings.has(input.name) || seenOneWayBindings.has(input.name + "Change")) {
        continue;
      }
